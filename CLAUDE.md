...
components (`.cols`, `.panel`, `.record`, `.counter`, `.trap`, `.capture`) live
in `site.css` under their own section, and the shared rules above that section
apply. Folding the shell into `_layouts/default.html` is optional cleanup, not a
fix.
...
form and nav, and keeps records from breaking across pages. Those rules are
scoped to `body.eol`, which `eol.html` sets. A typical finding runs two or three
pages.
